...
<script>
import { dic, pipmonthInit, pipmonthsave, pipmonthUpdate } from '@/api/fill'
...
export default {
  ...
  methods: {
    ...
    // 数据回显
    update() {
      pipmonthUpdate(this.$route.query.id).then((res) => {
        if (res.code === 0) {
    ...
    close() {
      this.$router.push('/pipelinemonth/productList')
    },
    // 新增保存
    createData() {
