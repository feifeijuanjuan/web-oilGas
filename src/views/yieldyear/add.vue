<template>
  <div class="app-container">
    <div class="form-add"><span class="first">年度产量信息填报</span>
...
<script>
import { yieldyearSave, yieldyearUpdate } from '@/api/fill'
...
export default {
  ...
  methods: {
    ...
    // 数据回显
    update() {
      yieldyearUpdate(this.$route.query.id).then((res) => {
        if (res.code === 0) {
    ...
    close() {
      this.$router.push('/yieldyear/list')
    },
    // 新增保存
    createData() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          yieldyearSave(this.editForm).then((res) => {
            if (res.code === 0) {
              this.$notify({
                message: '保存成功',
                type: 'success',
