<template>
  <div class="app-container">
    <div class="form-add"><span class="first">管道企业填报</span>
      ...
      <span class="first">管道企业调峰填报</span><span class="first-line">></span>
...
<script>
import { pipelinepeekdaySave, pipelinepeekdayUpdate, pipelinemonthInit, dic,pipelinepeekdayChange } from '@/api/fill'
...
export default {
  ...
  methods: {
    ...
    // 数据回显
    update() {
      pipelinepeekdayUpdate(this.$route.query.id).then((res) => {
        if (res.code === 0) {
    ...
    close() {
      this.$router.push('/pipelinepeak/list')
    },
    createData() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          pipelinepeekdaySave(this.editForm).then((res) => {
            if (res.code === 0) {
              this.$notify({
                message: '保存成功',
                type: 'success',
    ...
    updateData() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          pipelinepeekdayChange(this.editForm).then((res) => {
            if (res.code === 0) {
              this.$notify({
                message: '修改成功',
                type: 'success',
