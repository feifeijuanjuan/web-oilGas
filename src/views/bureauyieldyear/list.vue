...
import { bureauyieldyearList, bureauyieldyearSwitchs } from '@/api/fill'
...
export default {
  ...
  methods: {
    ...
    list(val, pageSize) {
      ...
      bureauyieldyearList(params).then((res) => {
        if (res.code === 0) {
    ...
    handleAdd() {
      ...
      this.$router.push({ path: '/bureauyieldyearAdd', query: params })
    },
    // 编辑
    handleEdit() {
    ...
    handleDel() {
      if (this.selectedRows.length > 0) {
        this.$confirm('确认删除选择数据吗?', '提示', {
          ...
          bureauyieldyearSwitchs(params).then((res) => {
            if (res.code === 0) {
              this.$notify({
                type: 'success',
                message: '删除成功!',
