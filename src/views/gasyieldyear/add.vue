<template>
  <div class="app-container">
    <div class="form-add"><span class="first">气田年度产量信息填报</span>
        ...
        <el-row>
          <!--          <el-col :span="12">
                      <el-form-item label="企业名称" class="no-unit">
                        <el-input  v-model="editForm.enterName" disabled>
                        </el-input>
                      </el-form-item>
                    </el-col>-->
          <el-col :span="12">
...
<script>
import { gasyieldyearInit, gasyieldyearSave, gasyieldyearUpdate } from '@/api/fill'
...
export default {
  ...
  methods: {
    gasyieldyearInit() {
      gasyieldyearInit().then((res) => {
        if (res.success) {
          this.editForm.enterName = res.data.zuzhijigou
          this.gasAreaAry=res.data.quyu
        } else {
          this.$notify({
            message: '网络请求失败',
            type: 'error',
    ...
    // 数据回显
    update() {
      gasyieldyearUpdate(this.$route.query.id).then((res) => {
        if (res.code === 0) {
    ...
    close() {
      this.$router.push('/gasyieldyear/list')
    },
    // 新增保存
    createData() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          gasyieldyearSave(this.editForm).then((res) => {
            if (res.code === 0) {
              this.$notify({
                message: '保存成功',
                type: 'success',
