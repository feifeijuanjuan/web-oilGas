<template>
  <div class="app-container">
    <div class="form-add"><span class="first">油气田企业填报</span>
      ...
      <span class="first">油田生产基地月产量填报</span
      ><span class="first-line">></span>
...
<script>
import {oilbasemonthUpdate, oilbasemonthSave, oilgasdayInit,oilbasemonthChange } from '@/api/fill'
  ...
  methods: {
    ...
    update() {
      return new Promise((resolve, reject) => {
        oilbasemonthUpdate(this.$route.query.id).then((res) => {
          if (res.code === 0) {
    ...
    close() {
      this.$router.push('/oilbasemonth/list')
    },
    // 新增保存
    createData() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          oilbasemonthSave(this.editForm).then((res) => {
            if (res.code === 0) {
              this.$notify({
                message: '保存成功',
                type: 'success',
    ...
    updateData() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          oilbasemonthChange(this.editForm).then((res) => {
            if (res.code === 0) {
              this.$notify({
                message: '修改成功',
                type: 'success',
