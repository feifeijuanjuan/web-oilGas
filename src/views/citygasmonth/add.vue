<template>
  <div class="app-container">
    <div class="form-add"><span class="first">城市燃气按日填报</span>
      <span class="first-line">></span>
      <span class="first">非供暖季按月填报</span
      ><span class="first-line">></span>
...
<!--              <el-select v-model="editForm.enterName">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="居民用气量天然气月消费量">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="民生用气量天然气月消费量">
          ...
          <el-col :span="12">
            <el-form-item label="社会车辆用气天然气月消费量">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="商业用户天然气月消费量">
          ...
          <el-col :span="12">
            <el-form-item label="工业燃料天然气月消费量">
...
<script>
import { citygasmonthSave, citygasmonthUpdate, citygasyearInit, dic } from '@/api/fill'
...
export default {
  ...
  methods: {
    ...
    // 数据回显
    update() {
      citygasmonthUpdate(this.$route.query.id).then((res) => {
        if (res.code === 0) {
    ...
    close() {
      this.$router.push('/citygasmonth/list')
    },
    // 新增保存
    createData() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          citygasmonthSave(this.editForm).then((res) => {
            if (res.code === 0) {
              this.$notify({
                message: '保存成功',
                type: 'success',
