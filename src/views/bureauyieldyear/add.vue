<template>
  <div class="app-container">
    <div class="form-add"><span class="first">综合能源年度产量信息填报</span>
...
<script>
import { bureauyieldyearSave, bureauyieldyearUpdate } from '@/api/fill'
...
export default {
  ...
  methods: {
    ...
    // 数据回显
    update() {
      bureauyieldyearUpdate(this.$route.query.id).then((res) => {
        if (res.code === 0) {
    ...
    close() {
      this.$router.push('/bureauyieldyear/list')
    },
    // 新增保存
    createData() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          bureauyieldyearSave(this.editForm).then((res) => {
            if (res.code === 0) {
              this.$notify({
                message: '保存成功',
                type: 'success',
