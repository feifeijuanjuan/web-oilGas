...
import { productList, pipmonthSwitchs, dic, pipmonthInit } from '@/api/fill'
  ...
  data() {
    return {
      ...
      tableLabel: [
        ...
        { label: '管线名', param: 'pipelineName' },
        { label: '所属企业', param: 'enterName' },
        { label: '管线进油量(万吨)', param: 'pipelineInputVolume' },
        { label: '管线出油量(万吨)', param: 'pipelineOutputVolume' },
        { label: '管线管存量(万吨)', param: 'pipelineStock' },
        // { label: '管线累计输油(万吨)', param: 'pipelineCumulativeVolume', minWidth: 180 },
        // { label: '末站压力阈值(Mpa)', param: 'pressureThreshold', minWidth: 180 },
        { label: '末站压力实际值(Mpa)', param: 'pressureActualValue' }
    ...
    handleDel() {
      if (this.selectedRows.length > 0) {
        this.$confirm('确认删除选择数据吗?', '提示', {
          ...
          pipmonthSwitchs(params).then((res) => {
            if (res.code === 0) {
              this.$notify({
                type: 'success',
                message: '删除成功',
